App.registerModule('turnos', {
    ...
    renderHistItem(t) {
        ...
        else { info += `<span style="color:var(--accent);font-weight:900">#${escapeHtml(String(t.numero))}</span><span style="font-weight:600">${escapeHtml(t.nombre)}</span>${t.rut ? `<span style="font-size:10px;color:var(--text-light);margin-left:4px">${escapeHtml(t.rut)}</span>` : ''}${t.patente ? `<span style="font-size:10px;color:var(--text-light);margin-left:4px">[${escapeHtml(t.patente)}]</span>` : ''}${t.motivo ? `<span style="font-size:10px;padding:2px 6px;border-radius:4px;background:rgba(168,85,247,0.1);color:#c084fc;margin-left:4px">${escapeHtml(t.motivo)}</span>` : ''}`; }
