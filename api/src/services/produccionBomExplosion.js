...
const buildReglasExtras = (r) => {
    const flags = [];
    if (r.radio) flags.push('Radio');
    if (r.pulido) flags.push('Pulido');
    if (r.perforado) flags.push('Perforado');
    if (r.destaje) flags.push('Destaje');
    if (r.sacado) flags.push('Sacado');
    if (r.ventana) flags.push('Ventana');
    if (r.pintado) flags.push('Pintado');
    if (r.pintado_car) flags.push('Pintado Car');
    return flags;
};
...
const crearOrdenSimple = async (r, familia, estacionesFinales, m2, resultados, mecanizadoOperaciones, ordenToEstacionId) => {
    ...
    const margen = r.precio_unitario * r.cantidad - costo_total;
    const reglasExtras = buildReglasExtras(r);
    const espesorMM = Number(r.espesor_mm) || 6;
