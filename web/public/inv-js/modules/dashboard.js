const InvDashboard = {
    ...
    donutChart(items, w, h) {
        ...
        items.forEach(item => {
            ...
            if (sweep > 18) {
                ...
                labels += `<text x="${lx.toFixed(1)}" y="${(ly + 4).toFixed(1)}" text-anchor="middle" fill="white" font-size="16" font-weight="700" style="text-shadow:0 1px 3px rgba(0,0,0,0.3);pointer-events:none">${Math.round(pct * 100)}%</text>`;
            }
            angle += sweep;
        });
        return `<svg width="100%" height="100%" viewBox="0 0 ${w} ${h}" style="display:block;margin:0 auto;animation:donutSpin 0.8s ease-out"><g>${paths}</g>${labels}
            <text x="${cx}" y="${cy - 8}" text-anchor="middle" fill="var(--gray-800)" font-size="28" font-weight="900">${this.fmtNum(totalPlanchas)}</text>
            <text x="${cx}" y="${cy + 16}" text-anchor="middle" fill="var(--gray-400)" font-size="14" font-weight="600">Planchas</text></svg>`;
    },
    ...
    heatmap(data, rowLabels, colLabels, w, h) {
        ...
        const cellH = Math.min(45, ch / rowLabels.length);
        ...
        page.innerHTML = `
                ...
                <div class="inv-dash-grid-3" style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px;align-items:stretch">
                    ...
                    <div class="card" style="overflow:hidden;display:flex;flex-direction:column;min-height:420px">
                        ...
                        <div style="padding:10px 16px;overflow-x:auto;flex:1;display:flex;align-items:center;justify-content:center"><div style="width:340px;height:340px">${this.donutChart(donutItems, 340, 340)}</div></div>
                        <div style="padding:0 16px 8px;display:flex;flex-wrap:wrap;gap:4px">
