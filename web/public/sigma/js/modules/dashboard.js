    ...
    async renderData() {
        ...
        el.innerHTML = `
            ...
            <div style="position:relative;z-index:1;display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap">
                <div style="min-width:0"><h2 style="margin:0;font-size:15px;font-weight:800;color:white;letter-spacing:-0.5px">Dashboard</h2>
                <p style="margin:2px 0 0;font-size:10px;color:rgba(255,255,255,0.7);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">Panel de mantenimiento · ${rangoTexto}</p></div>
...
@media(max-width:768px){
    ...
    renderTopFallas(data) {
        ...
        const cards = data.slice(0, 5).map((item, i) => {
            ...
            return `<div style="background:${cfg.bg};border:2px solid ${cfg.border};border-radius:10px;padding:8px 4px;text-align:center;flex:1 1 0;min-width:0;box-shadow:0 2px 8px rgba(0,0,0,0.06);box-sizing:border-box;${cursor}" ${onclick}>
