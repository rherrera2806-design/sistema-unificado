    ...
    async render() {
        ...
        el.innerHTML = `
            ...
            <div style="display:flex;gap:6px;align-items:center">
                ...
                ${App.canCreate('machineTypes') ? '<button class="btn btn-primary" style="padding:5px 12px;font-size:12px" onclick="App.modules.machineTypes.showForm()"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-2px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg> Nuevo</button>' : ''}
                </div></div></div>
...
.mtype-row:hover{transform:translateX(2px);background:#f8fafc!important}
            ...
            <div class="card mtype-card">
                ...
                ${SigmaCards.generate({
                    title: (t) => t.nombre,
                    subtitle: (t) => `${(compByTipo[t.id] ? [...compByTipo[t.id]].length : 0)} componentes · ${(maqByTipo[t.id] || []).length} máquinas`,
                    fields: [
                        { label: 'ID', value: (t) => t.id },
                        { label: 'Componentes', value: (t) => { const ids = compByTipo[t.id] ? [...compByTipo[t.id]] : []; return ids.map(id => compMap[id]).filter(Boolean).map(c => c.nombre).join(', ') || 'Sin componentes'; } }
                    ],
                    actions: (t) => `${App.canEdit('machineTypes') ? `<button class="btn btn-sm btn-outline" onclick="App.modules.machineTypes.showForm(${t.id})">Editar</button>` : ''} ${App.canDelete('machineTypes') ? `<button class="btn btn-sm btn-danger" onclick="App.modules.machineTypes.delete(${t.id})">Eliminar</button>` : ''}`
                }, tipos)}
                </div>`}
                </div>
            </div>`;
