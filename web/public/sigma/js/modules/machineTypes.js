...
.mtype-row:hover{transform:translateX(2px);background:#f8fafc!important}
            ...
            <div class="card mtype-card">
                ...
                ${SigmaCards.generate({
                    ...
                    actions: (t) => `<button class="btn btn-sm btn-outline" onclick="App.modules.machineTypes.showForm(${t.id})">Editar</button> <button class="btn btn-sm btn-danger" onclick="App.modules.machineTypes.delete(${t.id})">Eliminar</button>`
                }, tipos)}
                </div>`}
                </div>
            </div>`;
