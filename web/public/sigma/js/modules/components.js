    ...
    _filter() {
        ...
        if (componentes.length === 0) {
            ...
            body.innerHTML = `<div class="sigma-table-wrap"><table><thead><tr><th>ID</th><th>Nombre</th><th>Descripción</th><th>Usado en Tipos</th><th>Repuestos</th><th>Acciones</th></tr></thead>
                ...
                ${SigmaCards.generate({
                    title: (c) => c.nombre,
                    subtitle: (c) => { const links = this._allLinks.filter(l => l.componente_id === c.id); const tipos = links.map(l => this._allTipos.find(t => t.id === l.tipo_id)).filter(Boolean); const reps = this._allRepuestos.filter(s => s.componente_id === c.id).length; return `${tipos.length} tipos · ${reps} repuestos`; },
                    fields: [
