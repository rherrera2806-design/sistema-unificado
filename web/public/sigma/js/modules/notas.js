App.registerModule('notas', {
    ...
    async render() {
        ...
        el.innerHTML = `
            ...
            <div style="position:relative;z-index:1;display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap"><div><h2 style="margin:0;font-size:15px;font-weight:800;color:white;letter-spacing:-0.5px">Notas</h2>
            <p style="margin:2px 0 0;font-size:10px;color:rgba(255,255,255,0.7)">Comunicacion entre turnos</p></div>
    ...
    renderNotas(data) {
        ...
        for (const n of data) {
            ...
            html += '<div style="padding:16px;border-bottom:1px solid var(--border);display:flex;gap:16px;align-items:flex-start;' + rowBg + '">'
                ...
                + markReadBtn
                + (App.canEdit('notas') ? '<button class="btn btn-sm btn-outline" title="Editar" onclick="App.modules.notas.showForm(' + n.id + ')"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>' : '')
                + (App.canDelete('notas') ? '<button class="btn btn-sm btn-danger" title="Eliminar" onclick="App.modules.notas.delete(' + n.id + ')"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>' : '')
                + '</div>'
                + '</div>';
        }
