...
const Asistencia = {
    ...
    async render() {
        ...
        if (!this._initialized || !document.getElementById('ast-content')) {
            ...
            el.innerHTML = '<style>'
                ...
                + '.ranking-name{font-size:9px!important;min-height:22px}'
                + '}'
    ...
    renderHeroFilters(tab) {
        ...
        if (tab === 'trabajadores') {
            ...
            container.innerHTML = '<div style="position:relative">'
                ...
                + `<button onclick="Asistencia.cargarAsistencia()" class="btn btn-accent" style="white-space:nowrap;padding:8px 14px;font-size:12px">${BTN.CARGAR}</button>`;
        } else {
            container.innerHTML = '';
        }
    },
    ...
    renderTotalPermisos(permisos) {
        ...
        container.innerHTML = '<div class="m-stats" style="grid-template-columns:repeat(4,1fr)">'
            + '<div class="m-stat-card stat-info" style="flex-direction:row;align-items:center;gap:8px;padding:6px 10px;min-height:48px"><div class="m-stat-icon" style="background:linear-gradient(135deg,#3b82f615,#3b82f630)">' + iconClock + '</div><div style="display:flex;flex-direction:column;min-width:0"><div class="m-stat-value" style="color:#3b82f6;font-size:15px">' + totalHoras + 'h</div><div class="m-stat-label">Total Horas</div></div></div>'
            + '<div class="m-stat-card stat-green" style="flex-direction:row;align-items:center;gap:8px;padding:6px 10px;min-height:48px"><div class="m-stat-icon" style="background:linear-gradient(135deg,#22c55e15,#22c55e30)">' + iconCheck + '</div><div style="display:flex;flex-direction:column;min-width:0"><div class="m-stat-value" style="color:#22c55e;font-size:15px">' + aprobados + '</div><div class="m-stat-label">Aprobados</div></div></div>'
            + '<div class="m-stat-card stat-amber" style="flex-direction:row;align-items:center;gap:8px;padding:6px 10px;min-height:48px"><div class="m-stat-icon" style="background:linear-gradient(135deg,#f59e0b15,#f59e0b30)">' + iconClock + '</div><div style="display:flex;flex-direction:column;min-width:0"><div class="m-stat-value" style="color:#f59e0b;font-size:15px">' + pendientes + '</div><div class="m-stat-label">Pendientes</div></div></div>'
            + '<div class="m-stat-card stat-purple" style="flex-direction:row;align-items:center;gap:8px;padding:6px 10px;min-height:48px"><div class="m-stat-icon" style="background:linear-gradient(135deg,#8b5cf615,#8b5cf630)">' + iconX + '</div><div style="display:flex;flex-direction:column;min-width:0"><div class="m-stat-value" style="color:#8b5cf6;font-size:15px">' + total + '</div><div class="m-stat-label">Registros</div></div></div>'
            + '</div>';
    },
