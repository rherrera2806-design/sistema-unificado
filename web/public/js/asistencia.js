...
const Asistencia = {
    ...
    renderHeroFilters(tab) {
        ...
        if (tab === 'trabajadores') {
            container.innerHTML = '<div style="display:flex;gap:6px;align-items:center">'
                ...
                + `<button onclick="Asistencia.importarExcelTrabajadores()" class="btn" style="background:#0891b2;color:white;white-space:nowrap;padding:8px 14px;font-size:12px;border:none;border-radius:8px;font-weight:600;cursor:pointer" title="${BTN.IMPORTAR} desde Excel/CSV">${BTN.IMPORTAR}</button>`
            ...
            container.innerHTML = '<div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">'
                + '<select id="ast-hero-mes" style="width:auto;padding:8px 12px;background:rgba(255,255,255,0.3);color:white;border:1px solid rgba(255,255,255,0.5);font-size:12px;border-radius:6px;outline:none;cursor:pointer">' + opts + '</select>'
            ...
            container.innerHTML = '<div style="position:relative">'
                + '<svg style="position:absolute;left:10px;top:50%;transform:translateY(-50%)" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.5)" stroke-width="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>'
                + '<input type="text" id="ast-hero-buscar" class="ast-input" placeholder="Buscar nombre o RUT..." oninput="Asistencia.debouncedBuscarTrabajadores()" style="padding-left:32px;width:100%;min-width:100px;max-width:160px;background:rgba(255,255,255,0.35);color:white;border:1px solid rgba(255,255,255,0.5);font-size:11px;border-radius:6px">'
                + '</div>'
    ...
    renderTotalPermisos(permisos) {
        ...
        container.innerHTML = '<style>'
            ...
            + '<div class="m-stat-card stat-info"><div class="m-stat-icon" style="background:linear-gradient(135deg,#3b82f615,#3b82f630)">' + iconClock + '</div><div style="display:flex;flex-direction:column;min-width:0;overflow:hidden"><div class="m-stat-value" style="color:#3b82f6">' + totalHoras + 'h</div><div class="m-stat-label">Total Horas</div></div></div>'
            + '<div class="m-stat-card stat-green"><div class="m-stat-icon" style="background:linear-gradient(135deg,#22c55e15,#22c55e30)">' + iconCheck + '</div><div style="display:flex;flex-direction:column;min-width:0;overflow:hidden"><div class="m-stat-value" style="color:#22c55e">' + aprobados + '</div><div class="m-stat-label">Aprobados</div></div></div>'
