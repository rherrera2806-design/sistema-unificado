App.registerModule('prod_reportes', {
            ...
            <div style="background:linear-gradient(135deg,#0f172a 0%,#1e3a5f 50%,#1e40af 100%);border-radius:16px;padding:8px 16px;margin-bottom:20px;position:relative;overflow:hidden;box-shadow:0 4px 20px rgba(15,23,42,.3)">
                ...
                <div style="position:relative;z-index:1;display:flex;justify-content:space-between;align-items:center">
                    <div>
                        ...
                        <p style="margin:2px 0 0;font-size:11px;color:rgba(255,255,255,.7)">Reporte de ordenes agrupadas por fecha de entrega</p>
                    </div>
        ...
        for (const fecha of fechas) {
                ...
                <div style="overflow-x:auto;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 10px 10px;margin-bottom:4px">
                    <table class="pr-table">
                        <thead><tr>
                            ...
                            <th>Descripción Padre</th><th>Espesor</th><th>Ancho</th><th>Alto</th>
            ...
            for (const o of fecha.ordenes) {
                ...
                html += `<tr>
                    ...
                    <td style="max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${o.descripcion_padre || ''}">${o.descripcion_padre || ''}</td>
    ...
    exportarExcel() {
        ...
        let csv = 'FECHA;OV;Item;Pend;Estado;Cliente;Descripcion Padre;Ancho;Alto;Unidades;M2;Kgs;Ruta;Tipo;Grupo\n';
        for (const f of this.datos.fechas) {
            for (const o of f.ordenes) {
                csv += [
                    f.fecha, o.ov, o.item, o.pend, o.estado,
                    `"${(o.cliente || '').replace(/"/g, '""')}"`,
                    `"${(o.descripcion_padre || '').replace(/"/g, '""')}"`,
