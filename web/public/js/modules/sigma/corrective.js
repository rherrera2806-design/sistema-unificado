            ...
            <div class="card corr-card">
                ...
                <div class="card-body" style="padding:0">
                    ...
                    ${SigmaCards.generate({
                        ...
                        badge: r => `<span class="sc-badge" style="background:${r.estado === 'Reparada' ? '#dcfce7;color:#166534' : '#fef9c3;color:#854d0e'}">${r.estado || 'En Mantención'}</span>`,
                        fields: [
                            { label: 'Fecha', value: r => App.formatDate(r.fecha_falla) },
                            { label: 'Responsable', value: r => r.responsable || '-' },
                            { label: 'Hs.Det.', value: r => r.horas_detencion },
                            { label: 'Descripción', value: r => (r.descripcion_falla || '-').substring(0, 50) + ((r.descripcion_falla || '').length > 50 ? '...' : '') }
                        ],
                        actions: r => `
                            <button class="btn btn-sm btn-info" onclick="App.modules.corrective.showDetail(${r.id})">Ver</button>
                            <button class="btn btn-sm btn-outline" onclick="App.modules.corrective.showForm(${r.id})">Editar</button>`,
                        cardClass: r => r.estado !== 'Reparada' ? 'sc-mantencion' : ''
                    }, filtered)}`}
                    </div>
                </div>
            </div>`;
