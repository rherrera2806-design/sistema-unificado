App.registerModule('inst_detalle', {
    ...
    renderDetalle() {
        ...
        el.innerHTML = '<style>'
            ...
            + '.det-actions{gap:6px}'
            + '.det-actions .btn{font-size:11px;padding:4px 10px}'
            + '}'
            + '</style>'
            ...
            + '<button class="btn btn-outline" style="color:white;border-color:rgba(255,255,255,0.3);background:rgba(255,255,255,0.1)" onclick="App.loadModule(\'instalaciones\')" title="Volver a instalaciones"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-2px"><polyline points="15 18 9 12 15 6"/></svg> Volver</button>'
            + '<div style="min-width:0"><h2 style="margin:0;font-size:15px;font-weight:800;color:white;letter-spacing:-0.5px">Instalacion #' + inst.id + '</h2>'
            + '<p style="margin:4px 0 0;font-size:10px;color:rgba(255,255,255,0.7);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + escapeHtml(inst.cliente) + ' &middot; ' + escapeHtml(inst.direccion || '') + '</p></div>'
            + '</div>'
            + '<div class="det-actions">' + badge + actionBtns + '</div>'
            + '</div></div>'
